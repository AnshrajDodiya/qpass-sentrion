...
class Cron extends \Sentrion\Controllers\Pages\Base {
    ...
    public function __construct() {
        ...
        sentrion('storage')->set('ONERROR', sentrion('utils')->errorHandler->getCronErrorHandler());

        if (!sentrion('utils')->database->initConnect(false)) {
            sentrion('response')->error(404);
        }

        if (sentrion('request')->isCli()) {
            sentrion('session')->set('active_user_id', sentrion('utils')->constants->DAEMON_OPERATOR_ID);
        }
        ...
        sentrion('log')->debug('cron construct for %s in %f.', sentrion('request')->getUri(), sentrion('request')->getTimer($this->timer));
    }

    public function beforeroute(): void {
        sentrion('log')->debug('operator %s with roles %s accessing %s', $this->operator->email, json_encode($this->operator->roles), sentrion('request')->getUri());
        ...
        sentrion('log')->debug('cron beforeroute for %s in %f.', sentrion('request')->getUri(), sentrion('request')->getTimer($timer));
    }
    ...
    private function loadCrons(): void {
        sentrion('router')->config('config/crons.ini');

        $crons = (array) sentrion('storage')->get('crons');
    ...
    public function execute(string $jobName): void {
        ...
        $instance->$method();
        sentrion('utils')->cron->printLogs($instance->getLog());
    }

    private function isDue(\DateTime $time, string $expression): bool {
        $parts = sentrion('utils')->cron->parseExpression($expression);
        if (!$parts) {
            return false;
        }

        foreach (sentrion('utils')->cron->parseTimestamp($time) as $i => $k) {
