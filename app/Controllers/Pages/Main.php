<?php

/**
 * sentrion ~ open-source security framework
 * Copyright (c) Sentrion Technologies Sàrl (https://www.sentrion.com)
 *
 * Licensed under GNU Affero General Public License version 3 of the or any later version.
 * For full copyright and license information, please see the LICENSE
 * Redistributions of files must retain the above copyright notice.
 *
 * @copyright     Copyright (c) Sentrion Technologies Sàrl (https://www.sentrion.com)
 * @license       https://opensource.org/licenses/AGPL-3.0 AGPL License
 * @link          https://www.sentrion.com Sentrion(tm)
 */

declare(strict_types=1);

namespace Sentrion\Controllers\Pages;

class Main extends \Sentrion\Controllers\Pages\Base {
    public string $page = 'main';

    public function getSearchResults(): array {
        $this->assertCanView();

        $query = sentrion('utils')->conversion->getStringRequestParam('query');

        return $this->controller->getSearchResults($query, $this->apiKey);
    }

    public function getCurrentTime(): array {
        $this->assertCanView();

        return $this->operator->isLoggedIn() ? $this->controller->getCurrentTime($this->operator) : [];
    }

    public function getConstants(): array {
        $this->assertCanView();

        return $this->controller->getConstants();
    }
}
