...
abstract class Endpoint {
    ...
    protected \Sentrion\Views\Json $response;
    ...
    //protected \Sentrion\Interfaces\ApiKeyAccessAuthorizationInterface $authorizationModel;
    ...
    public function __construct() {
        sentrion('storage')->set('ONERROR', function (): void {
            $this->handleInternalServerError();
        });
        sentrion('utils')->database->initConnect(false);

        $this->response = new \Sentrion\Views\Json();
        $this->responseType = sentrion('utils')->constants->SINGLE_RESPONSE_TYPE;
    }
    ...
    protected function identify(): void {
        $headers = sentrion('request')->getHeaders();
        ...
        $this->setError(400, sentrion('utils')->errorCodes->REST_API_KEY_DOES_NOT_EXIST);
    }

    protected function authenticate(): void {
        $apiKeyId = sentrion('models')->apiKeys->getKeyIdByHash($this->apiKeyString);
        ...
        $this->setError(401, sentrion('utils')->errorCodes->REST_API_KEY_IS_NOT_CORRECT);
    }
    ...
    /*protected function authorize(string $subjectId): void {
        ...
        $this->setError(403, sentrion('utils')->errorCodes->REST_API_NOT_AUTHORIZED);
    }*/
    ...
    protected function saveLogbook(): void {
        sentrion('entities')->logbook->addRecord(
            ...
            $this->apiKeyId,
            !isset($this->error) ? sentrion('utils')->constants->LOGBOOK_ERROR_TYPE_SUCCESS : sentrion('utils')->constants->LOGBOOK_ERROR_TYPE_CRITICAL_ERROR,
            //$ended,
        );
    }
    ...
    protected function setError(int $statusCode, int $errorCode): void {
        sentrion('router')->status($statusCode);
    ...
    private function parseBody(): void {
        $body = sentrion('request')->getBody();
        $this->body = json_decode($body, true) ?? [];
    }

    private function handleInternalServerError(): void {
        $errorData = sentrion('utils')->errorHandler->getErrorDetails();
        sentrion('utils')->errorHandler->saveErrorInformation($errorData);
