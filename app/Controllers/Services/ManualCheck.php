<?php

/**
 * sentrion ~ open-source security framework
 * Copyright (c) Sentrion Technologies Sàrl (https://www.sentrion.com)
 *
 * Licensed under GNU Affero General Public License version 3 of the or any later version.
 * For full copyright and license information, please see the LICENSE
 * Redistributions of files must retain the above copyright notice.
 *
 * @copyright     Copyright (c) Sentrion Technologies Sàrl (https://www.sentrion.com)
 * @license       https://opensource.org/licenses/AGPL-3.0 AGPL License
 * @link          https://www.sentrion.com Sentrion(tm)
 */

declare(strict_types=1);

namespace Sentrion\Controllers\Services;

class ManualCheck extends \Sentrion\Controllers\Services\Base {
    public function performSearch(int $apiKey): array {
        $params = sentrion('utils')->render->extractRequestParams(['token', 'search', 'type']);

        $pageParams = [
            'SEARCH_VALUES' => $params,
        ];

        $enrichmentKey = sentrion('utils')->apiKeys->getCurrentOperatorEnrichmentKeyString();
        $errorCode = sentrion('utils')->validators->validateSearch($params, $enrichmentKey);

        if ($errorCode) {
            $pageParams['ERROR_CODE'] = $errorCode;

            return $pageParams;
        }

        $type   = sentrion('utils')->conversion->getStringRequestParam('type');
        $search = sentrion('utils')->conversion->getStringRequestParam('search');

        $result = sentrion('controllers')->enrichment->enrichEntity($type, $search, null, $apiKey, $enrichmentKey);

        if (isset($result['ERROR_CODE'])) {
            $pageParams['ERROR_CODE'] = $result['ERROR_CODE'];

            return $pageParams;
        }

        $operatorId = sentrion('utils')->routes->getCurrentRequestOperator()->id;
        $this->saveSearch($search, $type, $operatorId);

        // TODO: return alert_list back in next release
        if (array_key_exists('alert_list', $result[$type])) {
            unset($result[$type]['alert_list']);
        }

        if ($type === 'phone') {
            unset($result[$type]['valid']);
            unset($result[$type]['validation_error']);
        }

        if ($type === 'email') {
            unset($result[$type]['data_breaches']);
        }

        $pageParams['RESULT'] = [$type => $result[$type]];

        return $pageParams;
    }

    private function saveSearch(string $query, string $type, int $operatorId): void {
        sentrion('models')->manualCheck->insertRecord($query, $type, $operatorId);
    }

    public function getSearchHistory(int $operatorId): ?array {
        return sentrion('models')->manualCheck->getLastByOperatorId($operatorId);
    }
}
