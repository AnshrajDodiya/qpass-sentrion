...
class Isps extends \Sentrion\Controllers\Services\Base {
    public function getList(int $apiKey): array {
        ...
        $result = $this->idMapIterate($map, sentrion('grids')->isps, $apiKey);

        $ids = array_column($result['data'], 'id');
        if ($ids && sentrion('utils')->variables->getRecalculateTotalsOnVisit()) {
            sentrion('models')->isp->updateTotalsByEntityIds($ids, $apiKey);
            $result['data'] = sentrion('models')->isp->refreshTotals($result['data'], $apiKey);
        }
        return $result;
    }

    public function getChart(int $apiKey): array {
        return sentrion('charts')->isps->getData($apiKey);
    }
    ...
    public function getTimeFrameTotal(array $ids, string $startDate, string $endDate, int $apiKey): array {
        return [
            ...
            'totals'            => sentrion('models')->isp->getTimeFrameTotal($ids, $startDate, $endDate, $apiKey),
        ];
    }
}
