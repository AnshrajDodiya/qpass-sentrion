...
class Context extends \Sentrion\Controllers\Services\Base {
    private \Sentrion\Models\Context\User $userModel;
    private \Sentrion\Models\Context\Ip $ipModel;
    private \Sentrion\Models\Context\Device $deviceModel;
    private \Sentrion\Models\Context\Email $emailModel;
    private \Sentrion\Models\Context\Phone $phoneModel;
    private \Sentrion\Models\Context\Event $eventModel;
    private \Sentrion\Models\Context\Session $sessionModel;

    private ?\Sentrion\Assets\Context $extraModel;

    private array $suspiciousWordsUrl;
    private array $suspiciousWordsUserAgent;
    private array $suspiciousWordsEmail;
    private array $suspiciousAsns;
    private array $aiBots;

    public function __construct() {
        $this->userModel    = new \Sentrion\Models\Context\User();
        $this->ipModel      = new \Sentrion\Models\Context\Ip();
        $this->deviceModel  = new \Sentrion\Models\Context\Device();
        $this->emailModel   = new \Sentrion\Models\Context\Email();
        $this->phoneModel   = new \Sentrion\Models\Context\Phone();
        $this->eventModel   = new \Sentrion\Models\Context\Event();
        $this->sessionModel = new \Sentrion\Models\Context\Session();

        $this->extraModel   = sentrion('assets')->context->getContextObj();

        $this->suspiciousWordsUrl       = sentrion('assets')->urlList->getList();
        $this->suspiciousWordsUserAgent = sentrion('assets')->userAgentList->getList();
        $this->suspiciousWordsEmail     = sentrion('assets')->emailList->getList();
        $this->suspiciousAsns           = sentrion('assets')->asnList->getList();
        $this->aiBots                   = sentrion('assets')->aiBotList->getList();
    }
    ...
    public function getContext(array $accountIds, int $apiKey): array {
        ...
        $ipDetails          = $this->ipModel->getContext($accountIds, $apiKey);         // 43
        $deviceDetails      = $this->deviceModel->getContext($accountIds, $apiKey);     // 6
        $emailDetails       = $this->emailModel->getContext($accountIds, $apiKey);
        $phoneDetails       = $this->phoneModel->getContext($accountIds, $apiKey);
        $eventDetails       = $this->eventModel->getContext($accountIds, $apiKey);      // 30
        //$domainDetails    = $this->domainModel->getContext($accountIds, $apiKey);
        $extraDetails       = $this->extraModel?->getContext($accountIds, $apiKey) ?? [];

        $timezoneName       = sentrion('models')->apiKeys->getTimezoneByKeyId($apiKey);
        $utcTime            = new \DateTime('now', sentrion('utils')->timezones->getUtcTimezone());
        $timezone           = sentrion('utils')->timezones->getTimezone($timezoneName);
        $offsetInSeconds    = $timezone->getOffset($utcTime);

        // get only suspicious sessions
        $sessionDetails     = $this->sessionModel->getContext($accountIds, $apiKey, $offsetInSeconds);  // 17
    ...
    // event_session_stat has:
    //  session duration in s
    //  ips count per session
    //  device count per session
    //  event_count per session
    //  country count per session
    //  new ips count per session
    //  new devices count per session
    //  http codes list and count
    //  event types list and count
    //  completed mark

    // most heavy: ip, event, session, device
    ...
    private function extendParams(array &$record): void {
        ...
        $record['le_with_long_local_part_length']   = $localPartLen > sentrion('utils')->constants->RULE_EMAIL_MAXIMUM_LOCAL_PART_LENGTH;
        $record['le_with_long_domain_length']       = $domainPartLen > sentrion('utils')->constants->RULE_EMAIL_MAXIMUM_DOMAIN_LENGTH;
        $record['le_email_in_blockemails']          = $record['le_blockemails'] ?? false;
        $record['le_is_invalid']                    = $record['le_exists'] && !sentrion('utils')->conversion->filterEmail($record['le_email']);
        ...
        $record['eip_suspicious_asn']   = boolval(count(array_intersect($record['eip_asn'], $this->suspiciousAsns)));
        $record['lp_fraud_detected']    = $record['lp_fraud_detected'] ?? false;
        $record['le_fraud_detected']    = $record['le_fraud_detected'] ?? false;

        $record['eup_has_rare_browser'] = (bool) count(array_diff($record['eup_browser_name'], array_keys(sentrion('utils')->constants->RULE_REGULAR_BROWSER_NAMES)));
        $record['eup_has_rare_os']      = (bool) count(array_diff($record['eup_os_name'], sentrion('utils')->constants->RULE_REGULAR_OS_NAMES));
        $record['eup_device_count']     = count($record['eup_device']);

        $record['eup_vulnerable_ua']    = false;
        $record['eup_empty_ua']         = false;

        foreach ($record['eup_ua'] as $useragent) {
            if ($useragent === null || $useragent === '') {
                $record['eup_empty_ua'] = true;
            } elseif (count($this->suspiciousWordsUserAgent)) {
                foreach ($this->suspiciousWordsUserAgent as $sub) {
                    if (stripos($useragent, $sub) !== false) {
                        $record['eup_vulnerable_ua'] = true;
                        break;
                    }
                }
            }
            if ($record['eup_vulnerable_ua'] && $record['eup_empty_ua']) {
                break;
            }
        }

        $record['eup_empty_lang']       = in_array(null, $record['eup_lang'], true) || in_array('', $record['eup_lang'], true);
        $record['eup_ai_bot']           = boolval(count(array_intersect($record['eup_os_name'], $this->aiBots)));
    }
    ...
    private function extendEventParams(array &$record): void {
        ...
        //$accountLoginFailId = sentrion('utils')->constants->ACCOUNT_LOGIN_FAIL_EVENT_TYPE_ID;
        $accountEmailChangeId               = sentrion('utils')->constants->ACCOUNT_EMAIL_CHANGE_EVENT_TYPE_ID;
        $accountPwdChangeId                 = sentrion('utils')->constants->ACCOUNT_PASSWORD_CHANGE_EVENT_TYPE_ID;
        ...
        $record['event_http_method_head']   = in_array(sentrion('utils')->constants->EVENT_REQUEST_TYPE_HEAD, $record['event_http_method']);
    ...
    private function getDaysDiff(?string $dt1, ?string $dt2): int {
        ...
        return sentrion('utils')->conversion->intVal($diff, 0);
    }
