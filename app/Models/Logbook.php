...
class Logbook extends \Sentrion\Models\Base {
    ...
    public function getLastSucceededEvent(int $apiKey): array {
        $params = [
            ':api_key'          => $apiKey,
            ':endpoint'         => '/sensor/',
            ':success'          => sentrion('utils')->constants->LOGBOOK_ERROR_TYPE_SUCCESS,
            ':validation_error' => sentrion('utils')->constants->LOGBOOK_ERROR_TYPE_VALIDATION_ERROR,
        ];
    ...
    public function rotateRequests(?int $apiKey): int {
        $params = [
            ':key'      => $apiKey,
            ':limit'    => sentrion('utils')->variables->getLogbookLimit(),
        ];
