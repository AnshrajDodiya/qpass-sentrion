...
class Event extends \Sentrion\Models\Base {
    ...
    public function getLastEventTimeBefore(int $time, int $accountId, int $apiKey): ?string {
        $params = [
            ':api_key'  => $apiKey,
            ':user_id'  => $accountId,
            ':time'     => sentrion('utils')->dateRange->getDateString($time),
        ];
    ...
    public function getEventDetails(int $id, int $apiKey): array {
        ...
        $results = sentrion('utils')->enrichment->calculateIpType($results);
        $results = sentrion('utils')->enrichment->calculateEmailReputation($results);
        ...
        if (count($results)) {
            ...
            $results['score_details'] = sentrion('models')->user->getApplicableRulesByAccountId($results['accountid'], $apiKey, true);
