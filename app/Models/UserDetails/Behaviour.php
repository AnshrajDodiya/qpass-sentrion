...
namespace Sentrion\Models\UserDetails;

class Behaviour extends \Sentrion\Models\Base {
    public function getDayDetails(int $userId, array $dateRange, int $apiKey): array {
        $params = [
            ...
            ':failed_login'     => sentrion('utils')->constants->ACCOUNT_LOGIN_FAIL_EVENT_TYPE_ID,
            ':success_login'    => sentrion('utils')->constants->ACCOUNT_LOGIN_EVENT_TYPE_ID,
            ':password_reset'   => sentrion('utils')->constants->ACCOUNT_PASSWORD_CHANGE_EVENT_TYPE_ID,
            ':seconds_day'      => sentrion('utils')->constants->SECONDS_IN_DAY,
            ':night_time_end'   => sentrion('utils')->constants->SECONDS_IN_HOUR * 5,
        ];
