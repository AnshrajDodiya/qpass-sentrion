...
class UserScore extends \Sentrion\Models\Base {
    ...
    public function updateUserScore(array $scores, array $details, int $accountId, int $apiKey): void {
        ...
        sentrion('log')->debug('score update for account %d; scores -- %s.', $accountId, json_encode($scores));
        sentrion('log')->debug('score update for account %d; details -- %s.', $accountId, json_encode($details));
    ...
    public function getScoreDetailsByUserId(int $id, int $apiKey, bool $all = false): array {
        ...
        usort($results, [\Sentrion\Utils\Sort::class, 'cmpSetUid']);
