...
class Ip extends \Sentrion\Models\Enrichment\Base {
    ...
    public function init(array $data): void {
        ...
        if (!sentrion('utils')->conversion->filterIp($this->ip) || !$this->validateCIDR($this->cidr)) {
    ...
    public function updateEntityInDb(int $entityId, int $apiKey): void {
        $previousIpData = sentrion('models')->ip->getIpById($entityId, $apiKey);
        ...
        $newIspId = sentrion('models')->isp->getIdByAsn($this->asn, $apiKey);
        ...
        $newIspModel = new \Sentrion\Models\Enrichment\Isp();
        ...
        if ($newIspId === null) {
            ...
            $newIspId = sentrion('models')->isp->insertRecord($newIspData, $apiKey);
        } else {
        ...
        $newCountryId = sentrion('models')->country->getCountryIdByIso($this->country);

        $countryRecord = sentrion('controllers')->country->getCountryById($newCountryId, $apiKey);
        if (!count($countryRecord)) {
            ...
            sentrion('models')->country->insertRecord($newCountryData, $apiKey);
        }
        ...
        sentrion('models')->ip->execQuery($query, $params);

        // update totals only after event_ip update!
        $ispIds = $this->slimIds([$previousIspId, $newIspId]);
        sentrion('models')->isp->updateTotalsByEntityIds($ispIds, $apiKey, true);

        $countryIds = $this->slimIds([$previousCountryId, $newCountryId]);
        sentrion('models')->country->updateTotalsByEntityIds($countryIds, $apiKey, true);
    }
}
