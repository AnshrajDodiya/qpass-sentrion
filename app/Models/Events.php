...
class Events extends \Sentrion\Models\Base {
    ...
    public function getDistinctAccountsVisitLimit(int $after, int $until): array {
        $params = [
            ':cursor'       => $after,
            ':next_cursor'  => $until,
            ':visit_limit'  => sentrion('utils')->variables->getUserQueueEventsLimit(),
        ];
    ...
    protected function applyLimit(string &$query, array &$queryParams): void {
        $start = sentrion('utils')->conversion->getIntRequestParam('start');
        $length = sentrion('utils')->conversion->getIntRequestParam('length');
    ...
    public function retentionDeletion(int $weeks, int $apiKey): int {
        ...
        $params = [
            ':api_key'  => $apiKey,
            ':weeks'    => $weeks,
            ':week_sec' => sentrion('utils')->constants->SECONDS_IN_WEEK,
        ];
