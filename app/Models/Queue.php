...
class Queue extends \Sentrion\Models\Base {
    protected string $tableName = 'queue_account_operation';
    ...
    public function setWaiting(array $ids): void {
        $this->setStatus(sentrion('utils')->constants->WAITING_QUEUE_STATUS_TYPE, $ids);
    }

    public function setFailed(array $ids): void {
        $this->setStatus(sentrion('utils')->constants->FAILED_QUEUE_STATUS_TYPE, $ids);
    }

    public function setCompleted(array $ids): void {
        $this->setStatus(sentrion('utils')->constants->COMPLETED_QUEUE_STATUS_TYPE, $ids);
    }

    public function setExecuting(array $ids): void {
        $this->setStatus(sentrion('utils')->constants->EXECUTING_QUEUE_STATUS_TYPE, $ids);
    }

    public function isInQueueStatus(int $accountId, string $action, int $key): array {
        $params = [
            ':account'  => $accountId,
            ':status'   => sentrion('utils')->constants->COMPLETED_QUEUE_STATUS_TYPE,
            ':key'      => $key,
            ':action'   => $action,
        ];
    ...
    public function actionIsInQueueProcessing(string $action, int $key): bool {
        $params = [
            ':failed'       => sentrion('utils')->constants->FAILED_QUEUE_STATUS_TYPE,
            ':completed'    => sentrion('utils')->constants->COMPLETED_QUEUE_STATUS_TYPE,
            ':key'          => $key,
            ':action'       => $action,
        ];
    ...
    public function addBatch(array $accounts, string $action): void {
        ...
        $params = [
            ':action'   => $action,
            ':waiting'  => sentrion('utils')->constants->WAITING_QUEUE_STATUS_TYPE,
        ];
    ...
    public function setFailedForStuckAction(string $action): void {
        $params = [
            ':action'   => $action,
            ':status'   => sentrion('utils')->constants->FAILED_QUEUE_STATUS_TYPE,
            ':stuck'    => sentrion('utils')->constants->EXECUTING_QUEUE_STATUS_TYPE,
        ];
    ...
    public function checkExecuting(string $action): array {
        $params = [
            ':action'   => $action,
            ':status'   => sentrion('utils')->constants->EXECUTING_QUEUE_STATUS_TYPE,
        ];
    ...
    public function getNextBatchInQueue(string $action, int $size): array {
        $params = [
            ':batch'    => $size,
            ':action'   => $action,
            ':status'   => sentrion('utils')->constants->WAITING_QUEUE_STATUS_TYPE,
        ];
