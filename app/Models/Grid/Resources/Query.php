...
class Query extends \Sentrion\Models\Grid\Base\Query {
    ...
    private function applyFileExtensions(string &$query, array &$queryParams): void {
        $fileTypeIds = sentrion('utils')->conversion->getArrayRequestParam('fileTypeIds');
        if (!$fileTypeIds) {
            return;
        }

        $list = sentrion('assets')->fileExtensionsList->getList();
        $keys = sentrion('assets')->fileExtensionsList->getKeys();
