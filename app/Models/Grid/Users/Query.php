...
namespace Sentrion\Models\Grid\Users;
...
class Query extends \Sentrion\Models\Grid\Base\Query {
    ...
    protected array $allowedColumns = ['score', 'accounttitle', 'created', 'lastseen', 'fraud', 'id'];
    ...
    private function applyScore(string &$query, array &$queryParams): void {
        $scoresRanges = sentrion('utils')->conversion->getArrayRequestParam('scoresRange');
        if (!$scoresRanges) {
            return;
        }
        ...
        foreach ($scoresRanges as $key => $scoreBase) {
            ...
            $queryParams[':score_base_' . $key] = sentrion('utils')->conversion->intValCheckEmpty($scoreBase, 0);
        }
