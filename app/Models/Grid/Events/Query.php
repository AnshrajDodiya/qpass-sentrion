...
namespace Sentrion\Models\Grid\Events;

class Query extends \Sentrion\Models\Grid\Base\Query {
    ...
    private function applySearch(string &$query, array &$queryParams): void {
        ...
        $search = sentrion('utils')->conversion->getDictionaryRequestParam('search');
        $searchConditions = '';
        ...
        if (isset($search['value']) && is_string($search['value']) && $search['value'] !== '') {
            if (sentrion('utils')->conversion->filterIp($search['value'])) {
                $searchConditions .= (
                    ' AND
                    (
                ...
                $queryParams[':search_value'] = '%' . $search['value'] . '%';
                $queryParams[':offset'] = strval(sentrion('utils')->timezones->getCurrentOperatorOffset());
            }
        }
    ...
    private function applyEventTypes(string &$query, array &$queryParams): void {
        $eventTypeIds = sentrion('utils')->conversion->getArrayRequestParam('eventTypeIds');
        if (!$eventTypeIds) {
            return;
        }
    ...
    private function applyDeviceTypes(string &$query, array &$queryParams): void {
        $deviceTypes = sentrion('utils')->conversion->getArrayRequestParam('deviceTypes');
        if (!$deviceTypes) {
            return;
        }
        ...
        foreach ($deviceTypes as $key => $deviceType) {
            if ($deviceType === 'other') {
                ...
                foreach (sentrion('utils')->constants->DEVICE_TYPES as $device) {
    ...
    private function applyRules(string &$query, array &$queryParams): void {
        $ruleUids = sentrion('utils')->conversion->getArrayRequestParam('ruleUids');
        if (!$ruleUids) {
            return;
        }
