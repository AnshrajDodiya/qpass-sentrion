...
namespace Sentrion\Models\Grid\Blacklist;
...
class Query extends \Sentrion\Models\Grid\Base\Query {
    ...
    private function applyEntityTypes(string &$query, array &$queryParams): void {
        ...
        $entityTypeIds = sentrion('utils')->conversion->getArrayRequestParam('entityTypeIds');
        if ($entityTypeIds) {
            $clauses = [];

            foreach ($entityTypeIds as $key => $entityTypeId) {
                $clauses[] = 'extra.type = :entity_type_' . $key;
                $queryParams[':entity_type_' . $key] = strtolower(sentrion('utils')->constants->ENTITY_TYPES[$entityTypeId]);
            }
