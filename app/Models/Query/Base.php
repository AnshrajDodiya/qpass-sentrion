...
class Base {
    ...
    protected array $tables = [
        'event_ip'          => \Sentrion\Models\Query\Ips::class,
        'event_account'     => \Sentrion\Models\Query\Users::class,
        'event_url'         => \Sentrion\Models\Query\Urls::class,
        'event_query'       => \Sentrion\Models\Query\Queries::class,
        'event_referer'     => \Sentrion\Models\Query\Referers::class,
        'event_session'     => \Sentrion\Models\Query\Sessions::class,
        'event_device'      => \Sentrion\Models\Query\Devices::class,
    ];
    ...
    protected function parseSelector(string $selector): void {
        ...
        foreach ($parts as $part) {
            ...
            if (str_starts_with($elem, 'limit=')) {         // limit
                $pp = explode('=', $elem);
                if (count($pp) === 2) {
                    $limit = sentrion('utils')->conversion->intVal($pp[1]);
            ...
            //} elseif (str_starts_with($elem, 'group=')) {
                ...
                $pp = explode('=', $elem);
                if (count($pp) === 2) {
                    $offset = sentrion('utils')->conversion->intVal($pp[1]);
    ...
    public function get(): object {
        ...
        return sentrion('entities')->$model->buildFromArray($result, $this->key);
    }

    public function execQuery(string $query, ?array $params): array|int|null {
        return sentrion('utils')->database->getDb()->exec($query, $params);
    }
}
