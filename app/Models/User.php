...
class User extends \Sentrion\Models\Base implements \Sentrion\Interfaces\ApiKeyAccessAuthorizationInterface, \Sentrion\Interfaces\ApiKeyAccountAccessAuthorizationInterface {
    ...
    public function deleteAllUserData(int $userId, int $apiKey): void {
        ...
        try {
            $entities = sentrion('models')->events->uniqueEntitiesByUserId($userId, $apiKey);

            $db->begin();
            $db->exec($queries, array_fill(0, 6, $params));

            // force update totals for ips before isps and countries!
            sentrion('models')->ip->updateTotalsByEntityIds($entities['ip_ids'], $apiKey, true);
            sentrion('models')->isp->updateTotalsByEntityIds($entities['isp_ids'], $apiKey, true);
            sentrion('models')->country->updateTotalsByEntityIds($entities['country_ids'], $apiKey, true);
            sentrion('models')->resource->updateTotalsByEntityIds($entities['url_ids'], $apiKey, true);
            sentrion('models')->domain->updateTotalsByEntityIds($entities['domain_ids'], $apiKey, true);
            // it is always a force update
            sentrion('models')->phone->updateTotalsByValues($entities['phone_numbers'], $apiKey);

            $db->commit();
        } catch (\Exception $e) {
            $db->rollback();
            sentrion('log')->error('failed to delete user: %s.', $e->getMessage());
            throw $e;
        }
    }
    ...
    public function getApplicableRulesByAccountId(int $id, int $apiKey, bool $all = false): array {
        ...
        usort($results, [\Sentrion\Utils\Sort::class, 'cmpScore']);
