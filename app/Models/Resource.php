...
class Resource extends \Sentrion\Models\Base implements \Sentrion\Interfaces\ApiKeyAccessAuthorizationInterface {
    protected string $tableName = 'event_url';

    public function getResourceById(int $resourceId, int $apiKey): array {
        $query = (
            'SELECT
                url,
                lastseen,
    ...
    public function getTimeFrameTotal(array $ids, string $startDate, string $endDate, int $apiKey): array {
        ...
        $params[':field_edit'] = sentrion('utils')->constants->FIELD_EDIT_EVENT_TYPE_ID;
        $query = (
            "SELECT
                event.url   AS id,
                COUNT(*)    AS cnt
    ...
    public function updateAllTotals(int $apiKey): int {
        $params = [
            ':key'          => $apiKey,
            ':field_edit'   => sentrion('utils')->constants->FIELD_EDIT_EVENT_TYPE_ID,
        ];
