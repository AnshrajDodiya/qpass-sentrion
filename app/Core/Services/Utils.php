...
class Utils extends BaseAggregator {
    protected string $namespace = '\\Sentrion\\Utils\\%s';
    ...
    protected function createObject(string $name, string $className, bool $getFullClass): object {
        if ($name === 'constants') {
            return new \Sentrion\Core\Services\StaticProxyConstants($this->getClassName($className, $getFullClass));
        }
    ...
    public function nowForCurrentOperator(): string {
        $time = $this->nowUtc();
        sentrion('utils')->timezones->localizeForActiveOperator($time);

        return $time;
    }

    public function nowUtc(): string {
        return gmdate(sentrion('utils')->timezones->FORMAT);
    }
}
