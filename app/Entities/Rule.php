...
class Rule {
    ...
    public static function getById(string $uid, int $key): ?self {
        $rule = sentrion('models')->operatorsRules->getRuleWithOperatorValue($uid, sentrion('utils')->constants->PRIMARY_RULES_SET_ID, $key);

        return self::getFromQuery($rule, $key);
    }

    public static function getFromQuery(array $data, int $key): self {
        return new \Sentrion\Entities\Rule(
            $data['id'],
            $data['value'],
            $data['created_at'],
            $data['proportion'],
    ...
    public function localizeTimestamps(?string $timezone = null): void {
        ...
        foreach ($this->tsFields as $prop) {
            if (property_exists($this, $prop) && $this->$prop) {
                $this->$prop = sentrion('utils')->timezones->localizeTimestamp($this->$prop, $utc, $timezone, false);
            }
        }
    }
}
