...
class User extends \Sentrion\Entities\Single {
    protected int $id;
    ...
    public static function getById(int $id, int $key): ?self {
        $model = new \Sentrion\Models\Query\Users($key);
    ...
    public function setWhitelist(): void {
        $this->fraud = false;
        sentrion('models')->user->updateFraudFlag([$this->id], $this->key, false);
    }

    public function setBlacklist(): void {
        $this->fraud = true;
        sentrion('models')->user->updateFraudFlag([$this->id], $this->key, true);
    }

    // immediately!
    public function delete(): void {
        sentrion('models')->user->deleteAllUserData($this->id, $this->key);
    }

    public function isScheduledForDeletion(): array {
        [$scheduled, $status] = sentrion('models')->queue->isInQueueStatus($this->id, sentrion('utils')->constants->DELETE_USER_QUEUE_ACTION_TYPE, $this->key);
    ...
    public function isScheduledForBlacklist(): array {
        [$scheduled, $status] = sentrion('models')->queue->isInQueueStatus($this->id, sentrion('utils')->constants->BLACKLIST_QUEUE_ACTION_TYPE, $this->key);
