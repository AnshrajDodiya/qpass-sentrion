...
class Cron {
    ...
    public static function sendBlacklistReportPostRequest(array $hashes, string $enrichmentKey, int $apiKey): string {
        ...
        $response = sentrion('utils')->network->sendApiRequest($postFields, '/global_alert_report', 'POST', $enrichmentKey, $apiKey);

        return $response->error ?? '';
    }

    public static function checkTimezone(string $timezone): bool {
        $hour = (new \DateTime('now', sentrion('utils')->timezones->getTimezone($timezone)))->format('H');
        $hour = sentrion('utils')->conversion->intValCheckEmpty($hour, 0);
    ...
    public static function sendUnreviewedItemsReminderEmail(string $name, string $email, int $reviewCount): bool {
        ...
        $subject = sentrion('storage')->get('UnreviewedItemsReminder_email_subject');
        $subject = sprintf($subject, $reviewCount);

        $message = sentrion('storage')->get('UnreviewedItemsReminder_email_body');
        $url = sentrion('utils')->variables->getHostWithProtocolAndBase();
        $message = sprintf($message, $name, $email, $reviewCount, $url);

        sentrion('utils')->mailer->send($name, $email, $subject, $message);
    ...
    public static function parseTimestamp(\DateTime $time): array {
        return [
            sentrion('utils')->conversion->intValCheckEmpty($time->format('i'), 0), // minute
            sentrion('utils')->conversion->intValCheckEmpty($time->format('H'), 0), // hour
            sentrion('utils')->conversion->intValCheckEmpty($time->format('d'), 1), // day of month
            sentrion('utils')->conversion->intValCheckEmpty($time->format('m'), 1), // month
            sentrion('utils')->conversion->intValCheckEmpty($time->format('w'), 0), // day of week
        ];
    }
        ...
        foreach ($expressionParts as $i => $field) {
            ...
            foreach ($fieldParts as $part) {
                ...
                if ($start === '*') {
                    ...
                    $start = sentrion('utils')->conversion->intValCheckEmpty($start, 0);
                    $end = sentrion('utils')->conversion->intValCheckEmpty($end, $start);
                }
                $step = sentrion('utils')->conversion->intValCheckEmpty($step, 0);
