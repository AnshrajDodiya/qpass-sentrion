...
class Variables {
    public static function getDB(): ?string {
        return getenv('DATABASE_URL') ?: sentrion('storage')->get('DATABASE_URL');
    }
    ...
    public static function getHosts(): array {
        $env = getenv('SITE');
        $conf = sentrion('storage')->get('SITE');
    ...
    public static function getDebugLevel(): int {
        return sentrion('utils')->conversion->intValCheckEmpty(static::get('DEBUG'), 0) ?? 0;
    }
    ...
    public static function getHostWithProtocol(): string {
        ...
        if (!str_starts_with($host, '[') && sentrion('utils')->conversion->filterIpGetType($host) === 6) {
            $host = '[' . $host . ']';
        }
    ...
    public static function getHostWithProtocolAndBase(): string {
        return self::getHostWithProtocol() . sentrion('storage')->get('BASE');
    }
    ...
    public static function getAvailableTimezones(): array {
        return array_intersect_key(sentrion('storage')->get('timezones'), array_flip(\DateTimeZone::listIdentifiers()));
    }

    public static function completedConfig(): bool {
        return
            (getenv('SITE') || sentrion('storage')->get('SITE')) &&
            (getenv('PEPPER') || sentrion('storage')->get('PEPPER')) &&
            (getenv('ENRICHMENT_API') || sentrion('storage')->get('ENRICHMENT_API')) &&
            (getenv('DATABASE_URL') || sentrion('storage')->get('DATABASE_URL'));
    }

    protected static function get(string $var, bool $useConstant = false): mixed {
        return getenv($var) ?: sentrion('storage')->get($var) ?: ($useConstant ? sentrion('utils')->constants->$var : null);
    }

    protected static function getInt(string $var): int {
        return sentrion('utils')->conversion->intValCheckEmpty(static::get($var, true), sentrion('utils')->constants->$var);
    }

    protected static function getBool(string $var, bool $default = false): bool {
        return sentrion('utils')->conversion->filterBool(static::get($var) ?? 'false') ?? $default;
    }
}
