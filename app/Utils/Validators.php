...
class Validators {
    ...
    protected static function getSafeInt(string $key, array $params): ?int {
        return isset($params[$key]) ? sentrion('utils')->conversion->intVal($params[$key]) : null;
    }

    protected static function getSafeArray(string $key, array $params): ?array {
        return isset($params[$key]) && is_array($params[$key]) ? $params[$key] : null;
    }
    ...
    protected static function validateApiKeyOwning(array $params): int|false {
        ...
        return !$keyId
            || !sentrion('utils')->access->checkCurrentOperatorApiKeyAccess($keyId)
            ? sentrion('utils')->errorCodes->API_KEY_ID_INVALID
            : false;
    }

    protected static function validateEmailNew(array $params): int|false {
        return !self::getSafeString('email', $params)
            || sentrion('models')->operator->getByEmail($params['email'])
            ? sentrion('utils')->errorCodes->EMAIL_ALREADY_EXIST
            : false;
    ...
    protected static function validateNewPasswordPresence(array $params): int|false {
        return !self::getSafeString('new-password', $params)
            ? sentrion('utils')->errorCodes->NEW_PASSWORD_DOES_NOT_EXIST
            : false;
    }

    protected static function validatePasswordLength(array $params): int|false {
        return !self::getSafeString('password', $params)
            || strlen($params['password']) < sentrion('storage')->get('MIN_PASSWORD_LENGTH')
            ? sentrion('utils')->errorCodes->PASSWORD_IS_TOO_SHORT
            : false;
    }

    protected static function validateNewPasswordLength(array $params): int|false {
        return !self::getSafeString('new-password', $params)
            || strlen($params['new-password']) < sentrion('storage')->get('MIN_PASSWORD_LENGTH')
            ? sentrion('utils')->errorCodes->PASSWORD_IS_TOO_SHORT
            : false;
    }
    ...
    protected static function validateCurrentPasswordPresence(array $params): int|false {
        return !self::getSafeString('current-password', $params)
            ? sentrion('utils')->errorCodes->CURRENT_PASSWORD_DOES_NOT_EXIST
            : false;
    }

    protected static function validateEmailChanged(array $params): int|false {
        return !self::getSafeString('email', $params)
            || strtolower($params['email']) === strtolower(sentrion('utils')->routes->getCurrentRequestOperator()->email ?? '')
            ? sentrion('utils')->errorCodes->EMAIL_IS_NOT_NEW
            : false;
    }
    ...
    protected static function validateEnrichedAttributes(array $params, array $attributes): int|false {
        return !isset($params['enrichedAttributes'])
            ...
            ? sentrion('utils')->errorCodes->UNKNOWN_ENRICHMENT_ATTRIBUTES
            : false;
    }
    ...
    protected static function validateReminderFreq(array $params): int|false {
        return !isset($params['review-reminder-frequency'])
            ...
            || !in_array($params['review-reminder-frequency'], sentrion('utils')->constants->NOTIFICATION_REMINDER_TYPES)
            ? sentrion('utils')->errorCodes->INVALID_REMINDER_FREQUENCY
            : false;
    }
    ...
    private static function validatePasswordConfirmationPresence(array $params): int|false {
        return !self::getSafeString('password-confirmation', $params)
            ? sentrion('utils')->errorCodes->PASSWORD_CONFIRMATION_MISSING
            : false;
    }
    ...
    private static function validatePasswordCompare(array $params): int|false {
        return !self::getSafeString('password-confirmation', $params)
            ...
            ? sentrion('utils')->errorCodes->PASSWORDS_ARE_NOT_EQUAL
            : false;
    }
    ...
    private static function validatePasswordRenewKeyPresence(?array $params): int|false {
        return $params === null
            ...
            ? sentrion('utils')->errorCodes->RENEW_KEY_DOES_NOT_EXIST
            : false;
    }
    ...
    private static function validateRetention(array $params): int|false {
        return !self::checkInterval('retention-policy', $params, 0, 12)
            ? sentrion('utils')->errorCodes->RETENTION_POLICY_DOES_NOT_EXIST
            : false;
    }
    ...
    private static function validateBlacklistThreshold(array $params): int|false {
        return (isset($params['blacklist-threshold'])
            ...
            ? sentrion('utils')->errorCodes->INVALID_BLACKLIST_THRESHOLD
            : false;
    }
    ...
    private static function validateReviewQueueThreshold(array $params): int|false {
        return !self::checkInterval('review-queue-threshold', $params, 0, 99)
            ? sentrion('utils')->errorCodes->INVALID_REVIEW_QUEUE_THRESHOLD
            : false;
    }
    ...
    private static function validateThresholdsCompare(array $params): int|false {
        ...
        return $reviewQueueThreshold === null
            ...
            ? sentrion('utils')->errorCodes->INVALID_THRESHOLDS_COMBINATION
            : false;
    }

    private static function validateRulesPresetId(array $params): int|false {
        return !array_key_exists($params['rules-preset'], sentrion('assets')->rulesPresets->getPresets())
            ? sentrion('utils')->errorCodes->INVALID_RULES_PRESET_ID
            : false;
    }

    private static function validateSearchEnrichment(?string $enrichmentKey): int|false {
        return !$enrichmentKey
            || !sentrion('utils')->variables->getEnrichmentApi()
            ? sentrion('utils')->errorCodes->ENRICHMENT_API_KEY_NOT_EXISTS
            : false;
    }

    private static function validateSearchType(array $params): int|false {
        $type = self::getSafeString('type', $params);
        $types = sentrion('storage')->get('manualCheck_form_types');

        return !$type
            || !is_array($types)
            || !array_key_exists($type, $types)
            ? sentrion('utils')->errorCodes->TYPE_DOES_NOT_EXIST
            : false;
    }

    private static function validateSearchValue(array $params): int|false {
        return !self::getSafeString('search', $params)
            ? sentrion('utils')->errorCodes->SEARCH_QUERY_DOES_NOT_EXIST
            : false;
    }

    private static function validateCurrentPassword(array $params): int|false {
        $operatorId = sentrion('utils')->access->getCurrentOperatorId();
        if ($operatorId === sentrion('utils')->constants->GUEST_OPERATOR_ID) {
            return sentrion('utils')->errorCodes->CURRENT_PASSWORD_IS_NOT_CORRECT;
        }

        if (!sentrion('models')->operator->verifyPassword($params['current-password'], $operatorId)) {
            return sentrion('utils')->errorCodes->CURRENT_PASSWORD_IS_NOT_CORRECT;
        }
    ...
    private static function validateBelongingCoOwner(array $params): int|false {
        ...
        $keyId = sentrion('utils')->access->getCurrentOperatorApiKeyId();

        if (!$operatorId || !$keyId) {
            return sentrion('utils')->errorCodes->OPERATOR_IS_NOT_A_CO_OWNER;
        }
    ...
    public static function validatePasswordRecovering(?array $params): int|false {
        ...
        $createdAt = sentrion('models')->forgotPassword->getUnusedByRenewKey($params['renewKey']);
        if (!$createdAt) {
            return sentrion('utils')->errorCodes->RENEW_KEY_IS_NOT_CORRECT;
        }

        $currentTime = time();
        $linkTime = strtotime($createdAt);
        $lifeTime = sentrion('storage')->get('RENEW_PASSWORD_LINK_TIME');

        if ($currentTime > $linkTime + $lifeTime) {
            return sentrion('utils')->errorCodes->RENEW_KEY_WAS_EXPIRED;
        }
