...
class Timezones {
    ...
    public static function getOperatorOffset(?\Sentrion\Entities\Operator $operator): int {
    ...
    public static function getCurrentOperatorOffset(): int {
        return self::getOperatorOffset(sentrion('utils')->routes->getCurrentRequestOperator());
    }
    ...
    public static function getLastNDaysRange(int $days = 1, int $offset = 0): array {
        $now = time();
        $daySeconds = sentrion('utils')->constants->SECONDS_IN_DAY;
    ...
    public static function getPreviousWeekRange(int $offset = 0): array {
        ...
        $date->setTime(0, 0, 0);
        $dow = sentrion('utils')->conversion->intValCheckEmpty($date->format('N'), 1);
    ...
    public static function getYesterdayRange(int $offset = 0): array {
        ...
        $date = new \DateTime();
        $date->setTimestamp($now - sentrion('utils')->constants->SECONDS_IN_DAY + $offset);

        $date->setTime(0, 0, 0);

        return [
            'endDate'   => date(self::FORMAT, $date->getTimestamp() + sentrion('utils')->constants->SECONDS_IN_DAY - $offset),
            'startDate' => date(self::FORMAT, $date->getTimestamp() - $offset),
            'offset'    => $offset,
        ];
    }
    ...
    public static function getSecondsSinceMonday(string $timestamp): int {
        ...
        $weekStart = strtotime(date('Y-m-d 00:00:00', $time)) - (($dow - 1) * sentrion('utils')->constants->SECONDS_IN_DAY);
    ...
    public static function getLatestWeekAgoDayRange(int $offset = 0): array {
        ...
        $week = sentrion('utils')->constants->SECONDS_IN_WEEK;
        $day = sentrion('utils')->constants->SECONDS_IN_DAY;
    ...
    public static function getCurWeekRange(int $offset = 0): array {
        ...
        $date->setTimestamp($now + $offset);
        $date->setTime(0, 0, 0);
        $dow = sentrion('utils')->conversion->intValCheckEmpty($date->format('N'), 0);
        $day = sentrion('utils')->constants->SECONDS_IN_DAY;
    ...
    public static function timezonesList(): array {
        $utcTime = new \DateTime('now', self::getUtcTimezone());
        $timezones = sentrion('utils')->variables->getAvailableTimezones();
