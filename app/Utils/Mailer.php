...
class Mailer {
    public static function send(?string $toName, string $toAddress, string $subj, string $msg, bool $html = false): array {
        $canSendEmail = sentrion('storage')->get('SEND_EMAIL');
        if (!$canSendEmail) {
            return [
                'success' => true,
        ...
        $toName = $toName ?? '';
        $data = null;
        if (sentrion('utils')->variables->getMailPassword()) {
    ...
    private static function sendByMailgun(string $toAddress, string $toName, string $subj, string $msg, bool $html): array {
        $fromName = sentrion('utils')->constants->MAIL_FROM_NAME;
        $smtpDebug = sentrion('storage')->get('SMTP_DEBUG');
        $fromAddress = sentrion('utils')->variables->getMailLogin();
        $mailLogin = sentrion('utils')->variables->getMailLogin();
        $mailPassword = sentrion('utils')->variables->getMailPassword();
        ...
        try {
            ...
            $mail->Host = sentrion('utils')->constants->MAIL_HOST;                   //Set the SMTP server to send through
    ...
    private static function sendByNativeMail(string $toAddress, string $toName, string $subj, string $msg): array {
        $sendMailPath = sentrion('utils')->constants->MAIL_SEND_BIN;
