...
class PagesClasses extends Base {
    ...
    public static function getSinglePageObject(string $name): ?\Sentrion\Core\Page {
        $obj = null;
        ...
        try {
            ...
            sentrion('log')->info('page file %s not found: %s.', $filename, $e->getMessage());
        }
        ...
        foreach ($iter as $file) {
            if ($file->isFile() && $file->getExtension() === 'php') {
                ...
                try {
                    ...
                    $failed[] = $name;
                    sentrion('log')->info('page file %s not found: %s.', $file->getFilename(), $e->getMessage());
                }
            }
        }

        // load file based route loader class
        $out['FileBasedPage'] = '\\Sentrion\\Core\\FileBasedPage';
