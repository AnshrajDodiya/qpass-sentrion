...
class RulesPresets {
    ...
    protected static function getFilenames(bool $core = true): array {
        ...
        sentrion('log')->debug('extracted filenames from dir %s: %s', $dir, json_encode($result));
    ...
    protected static function collectRulesFromFile(string $filename, array $uids, bool $core = true): ?array {
        ...
        foreach ($rules as $uid => $weight) {
            ...
            $weightInt = sentrion('utils')->constants->RULE_WEIGHT_MAP[$weight] ?? null;
    ...
    protected function loadData(): array {
        $presets = [];
        $uids = array_flip(array_column(sentrion('models')->rules->getAll(), 'uid'));
