...
class Enrichment {
    ...
    public static function calculateEmailReputation(array $records, string $fieldName = 'reputation'): array {
        ...
        for ($i = 0; $i < $iters; ++$i) {
            ...
            if ($record['data_breach'] !== null) {
                $reputationLevel = sentrion('utils')->conversion->intVal($record['data_breach'], 0) + sentrion('utils')->conversion->intVal(!$record['blockemails'], 0);
    ...
    public static function calculateRuleType(array $records): array {
        foreach ($records as &$record) {
            $record['type'] = sentrion('assets')->rules->getRuleTypeByUid($record['uid']);
        }
