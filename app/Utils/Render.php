...
class Render {
    // TODO:
    // sentrion('utils')->dictManager->load($page);
    ...
    public static function getUserScoreClass(?int $score, ?bool $fraud, ?string $addedToReview): array {
        ...
        if ($score !== null && $score >= sentrion('utils')->constants->USER_LOW_SCORE_INF && $score < sentrion('utils')->constants->USER_LOW_SCORE_SUP) {
            $cls = 'low';
        }

        if ($score !== null && $score >= sentrion('utils')->constants->USER_MEDIUM_SCORE_INF && $score < sentrion('utils')->constants->USER_MEDIUM_SCORE_SUP) {
            $cls = 'medium';
        }

        if ($score !== null && $score >= sentrion('utils')->constants->USER_HIGH_SCORE_INF) {
            $cls = 'high';
        }
    ...
    public static function extractRequestParams(array $params): array {
        ...
        foreach ($params as $key) {
            $result[$key] = sentrion('request')->getRequestParam($key);
        }
    ...
    protected static function getPageTitle(string $page): string {
        $title = sentrion('storage')->get(sprintf('%s_page_title', $page));
    ...
    public static function getBreadcrumbTitle(string $page): string {
        return sentrion('storage')->get(sprintf('%s_breadcrumb_title', $page)) ?? '';
    }
    ...
    public static function applyPageParams(array $params, string $page): array {
        ...
        $params['CURRENT_PATH'] = sentrion('request')->getPath();
        $params['CURRENT_PATTERN'] = sentrion('request')->getPattern();
        $params['SEARCH_PLACEHOLDER'] = sentrion('storage')->get(sprintf('%s_search_placeholder', $page)) ?? '';

        if ($errorCode) {
            $errorI18nCode = sprintf('error_%s', $errorCode);
            $errorMessage = sentrion('storage')->get($errorI18nCode);
            $params['ERROR_MESSAGE'] = $errorMessage;
        }

        if ($successCode) {
            $successI18nCode = sprintf('error_%s', $successCode);
            $successMessage = sentrion('storage')->get($successI18nCode);
        ...
        if ($currentOperator->isLoggedIn()) {
            ...
            $params += sentrion('controllers')->main->getCurrentTime($currentOperator);
        }

        $params['ALLOW_EMAIL_PHONE'] = sentrion('utils')->variables->getEmailPhoneAllowed();
        $params['FILE_PAGES'] = sentrion('assets')->pages->getMenuPages();
        ...
        $params['CSRF'] = sentrion('session')->get('csrf');
