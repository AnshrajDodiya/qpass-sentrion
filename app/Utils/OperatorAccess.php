...
class OperatorAccess {
    public static function addOperatorRole(string $role, int $operatorId): void {
        $roleId = sentrion('models')->roles->getRoleByValues($role)['id'] ?? null;
        if (!$roleId) {
            return;
        }
    ...
    public static function getPermissions(): array {
        $permissions = sentrion('models')->roles->getAllRolesWithPermissions();
    ...
    private static function rolesModel(): \Sentrion\Models\OperatorsRoles {
        return sentrion('models')->operatorsRoles;
    }
}
