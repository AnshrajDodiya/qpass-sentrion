...
use Sentrion\Utils\SystemMessages;
use PHPUnit\Framework\TestCase;

/**
 * Unit tests for Sentrion\Utils\SystemMessages.
 *
 * Covered:
...
final class SystemMessagesTest extends TestCase {
    public function testSyslogLineBuildsPriTimestampHostAppPidAndMessage(): void {
        ...
        // App and pid are embedded as "app[PID]:" with fixed host "sentrion".
        $this->assertStringContainsString(' sentrion ' . $app . '[' . $pid . ']: ', $line);
        ...
        $this->assertMatchesRegularExpression(
            '/^<\d+>[A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}\s+sentrion\s+/',
            $line
        );
    }
