...
class A03 extends \Sentrion\Assets\Rule {
    ...
    protected function prepareParams(array $params): array {
        ...
        if ($params['eup_device_count'] > 1 && count(array_unique($params['eip_country_id'])) > 1) {
            foreach (array_keys($params['event_device']) as $idx) {
                if (sentrion('utils')->rules->eventDeviceIsNew($params, $idx) && sentrion('utils')->rules->countryIsNewByIpId($params, $params['event_ip'][$idx])) {
