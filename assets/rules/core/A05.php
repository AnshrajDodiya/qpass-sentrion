...
class A05 extends \Sentrion\Assets\Rule {
    ...
    protected function prepareParams(array $params): array {
        ...
        $passwordChange = sentrion('utils')->constants->ACCOUNT_PASSWORD_CHANGE_EVENT_TYPE_ID;

        if ($params['eup_device_count'] > 1) {
            foreach (array_keys($params['event_device']) as $idx) {
                if ($params['event_type'][$idx] === $passwordChange && sentrion('utils')->rules->eventDeviceIsNew($params, $idx)) {
