...
class A04 extends \Sentrion\Assets\Rule {
    ...
    protected function prepareParams(array $params): array {
        ...
        if ($params['eup_device_count'] > 1 && $params['eip_unique_cidrs'] > 1) {
            foreach (array_keys($params['event_device']) as $idx) {
                if (sentrion('utils')->rules->eventDeviceIsNew($params, $idx) && sentrion('utils')->rules->cidrIsNewByIpId($params, $params['event_ip'][$idx])) {
