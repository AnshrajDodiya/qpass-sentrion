...
class A06 extends \Sentrion\Assets\Rule {
    ...
    protected function prepareParams(array $params): array {
        ...
        $pwdChange = sentrion('utils')->constants->ACCOUNT_PASSWORD_CHANGE_EVENT_TYPE_ID;

        if (count(array_unique($params['eip_country_id'])) > 1) {
            foreach ($params['event_type'] as $idx => $event) {
                if ($event === $pwdChange) {
                    if (sentrion('utils')->rules->countryIsNewByIpId($params, $params['event_ip'][$idx])) {
