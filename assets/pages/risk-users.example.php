...
// Use the sentrion query builder (auto-scoped to the current API key) to list
// entities with a trust score of 20 or higher. get()->data returns an array
// of \Sentrion\Entities\User objects.
$entities = sentrion('queries')->users->where('user_score', '>=', 20)->limit(50)->get()->data;
...
$rows = array_map(static function (\Sentrion\Entities\User $user): array {
    return [
        'id'        => $user->id,
        'userid'    => $user->userid,
